...
\begin{ej}
	...
	\begin{figure}[h]
		...
		\begin{tikzpicture}
		\node (H) at (0,3) {$H$};
		\node (ab) at (-1.5, 2) {$\langle AB\rangle$};
...
\begin{itemize}
	...
	\begin{itemize}
		\item Nos damos cuenta después de muchas cuentas que $\gen{AB^2} = \{1, AB^2 = A^3, A^2 = B^2, A\} = \gen{A} < H$ pero no es nuevo así que no ha habido suerte.
		\item Probamos con $\gen{AB} = \{1, AB, A^2 = B^2, AB^3\} < H$ y tenemos suerte porque hemos encontrado uno que no estaba. Ocurrirá que $\gen{AB} = \gen{AB^3}$.
		\item Nos quedan por probar $\gen{B}$ y $\gen{B^3}$. Está claro que serán subgrupos y no serán el total porque no hay $A$s pero ocurre que son el mismo subgrupo.
	\end{itemize}
	\item Así, hemos mirado todos los subgrupos generados por un elemento y hemos identificado los que son iguales. ¿Qué nos garantiza que grupos generados por dos elementos no serán alguno de los que hemos encontrado? Pues no sé.
\end{itemize}
\end{ej}
